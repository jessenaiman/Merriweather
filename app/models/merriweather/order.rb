...
#module Merriweather
  class Order < ActiveRecord::Base
    ...
    if Merriweather.user_class
      belongs_to :user, :class_name => Merriweather.user_class.to_s
    else
      belongs_to :user
    end

    belongs_to :bill_address, :foreign_key => :bill_address_id, :class_name => "Merriweather::Address"
    alias_attribute :billing_address, :bill_address

    belongs_to :ship_address, :foreign_key => :ship_address_id, :class_name => "Merriweather::Address"
    ...
    def self.by_customer(customer)
      joins(:user).where("#{Merriweather.user_class.table_name}.email" => customer)
    end
    ...
    def tax_zone
      zone_address = Merriweather::Config[:tax_using_ship_address] ? ship_address : bill_address
    ...
    def exclude_tax?
      return false unless Merriweather::Config[:prices_inc_tax]
    ...
    def create_tax_charge!
      Merriweather::TaxRate.adjust(self)
    end
    ...
    def process_payments!
      begin
        ...
        !!Merriweather::Config[:allow_checkout_on_gateway_error]
      end
    end
