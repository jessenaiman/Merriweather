module Merriweather
  class Product < ActiveRecord::Base
    ...
    #
    #   Merriweather::Product.taxons_id_eq(x)
    ...
    # If you need products only within one taxon use
    #
    #   Merriweather::Product.taxons_id_eq([x,y])
    ...
    #   joins(:product_groups).where('merriweather_product_groups_products.product_group_id' => product_group)
    # end
    ...
    add_search_scope :with_property_value do |property, value|
      properties = Merriweather::Property.table_name
      conditions = case property
      when String   then ["#{properties}.name = ?", property]
      when Property then ["#{properties}.id = ?", property.id]
      else               ["#{properties}.id = ?", property.to_i]
    ...
    add_search_scope :with_option_value do |option, value|
      ...
      select("DISTINCT merriweather_products.id").joins(:variants_including_master => :option_values).where(conditions)
    end
    ...
    add_search_scope :taxons_name_eq do |name|
      select("DISTINCT merriweather_products.id").joins(:taxons).where(Taxon.arel_table[:name].eq(name))
    end
