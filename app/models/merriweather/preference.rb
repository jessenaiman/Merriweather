class Merriweather::Preference < ActiveRecord::Base
  attr_accessible :name, :key, :value_type, :value

  validates :key, :presence => true
  validates :value_type, :presence => true

  scope :valid, lambda { where(Merriweather::Preference.arel_table[:key].not_eq(nil)).where(Merriweather::Preference.arel_table[:value_type].not_eq(nil)) }

  # The type conversions here should match
  # the ones in Merriweather::preferences::preferrable#convert_preference_value
  def value
