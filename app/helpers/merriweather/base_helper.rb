module Merriweather
  module BaseHelper
     # Defined because Rails' current_page? helper is not working when Merriweather is mounted at root.
    def current_merriweather_page?(url)
      path = request.fullpath.gsub(/^\/\//, '/')
      if url.is_a?(String)
        return path == url
      elsif url.is_a?(Hash)
        return path == merriweather.url_for(url)
      end
      return false
    end

    def link_to_cart(text = nil)
      return "" if current_merriweather_page?(cart_path)
    ...
    def variant_options(v, allow_back_orders = Merriweather::Config[:allow_backorders], include_style = true)
      list = v.options_text
    ...
    def meta_data_tags
      ...
      if meta[:description].blank? && object.kind_of?(Merriweather::Product)
        meta[:description] = strip_tags(object.description)
      end

      meta.reverse_merge!({
        :keywords => Merriweather::Config[:default_meta_keywords],
        :description => Merriweather::Config[:default_meta_description]
      })
    ...
    def logo(image_path=Merriweather::Config[:logo])
    ...
    def available_countries
      checkout_zone = Zone.find_by_name(Merriweather::Config[:checkout_zone])
    ...
    def seo_url(taxon, product = nil)
      return merriweather.nested_taxons_path(taxon.permalink) if product.nil?
    ...
    def image_style_from_method_name(method_name)
      if style = method_name.to_s.sub(/_image$/, '')
        possible_styles = Merriweather::Image.attachment_definitions[:attachment][:styles]
