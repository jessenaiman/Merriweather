...
describe Merriweather::OrderMailer do
  ...
  let(:order) { Merriweather::Order.new(:email => "customer@example.com") }
  let(:message) { Merriweather::OrderMailer.confirm_email(order) }
  ...
  context "#deliver" do
    before do
      ...
      Merriweather::MailMethod.stub :current => mail_method
    end
