...
describe Merriweather::Core::CalculatedAdjustments do

  let(:calculator) { mock_model(Merriweather::Calculator, :compute => 10, :[]= => nil) }

  it "should add has_one :calculator relationship" do
    assert Merriweather::ShippingMethod.reflect_on_all_associations(:has_one).map(&:name).include?(:calculator)
  end

  let(:tax_rate) { Merriweather::TaxRate.new({:calculator => calculator}, :without_protection => true) }
    ...
    context "when the calculator returns 0" do
      ...
      context "when adjustment is mandatory" do
        ...
        it "should create an adjustment" do
          Merriweather::Adjustment.count.should == 1
        end
      end
      ...
      context "when adjustment is not mandatory" do
        ...
        it "should not create an adjustment" do
          Merriweather::Adjustment.count.should == 0
        end
      end
    end
