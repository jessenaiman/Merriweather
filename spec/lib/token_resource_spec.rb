...
describe Merriweather::Core::TokenResource do
  let(:order) { Merriweather::Order.new }
  let(:permission) { mock_model(Merriweather::TokenizedPermission) }

  it 'should add has_one :tokenized_permission relationship' do
    assert Merriweather::Order.reflect_on_all_associations(:has_one).map(&:name).include?(:tokenized_permission)
  end
