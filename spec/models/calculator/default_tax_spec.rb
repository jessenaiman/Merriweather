...
describe Merriweather::Calculator::DefaultTax do
  let!(:tax_category) { create(:tax_category, :tax_rates => []) }
  let!(:rate) { mock_model(Merriweather::TaxRate, :tax_category => tax_category, :amount => 0.05) }
  let!(:calculator) { Merriweather::Calculator::DefaultTax.new({:calculable => rate}, :without_protection => true) }
