...
describe Merriweather::Calculator::FlexiRate do
  let(:calculator) { Merriweather::Calculator::FlexiRate.new }
  let(:order) { mock_model Merriweather::Order, :line_items => [mock_model(Merriweather::LineItem, :amount => 10, :quantity => 4), mock_model(Merriweather::LineItem, :amount => 20, :quantity => 6)] }
  ...
  context "compute" do
    ...
    it "should allow creation of new object with all the attributes" do
      Merriweather::Calculator::FlexiRate.new(:preferred_first_item => 1, :preferred_additional_item => 1, :preferred_max_items => 1)
    end
  end
end
