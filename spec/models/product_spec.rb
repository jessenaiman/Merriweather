...
describe Merriweather::Product do
  before(:each) do
  ...
  context "#on_hand=" do
    it "should not complain of a missing master" do
      product = Merriweather::Product.new
      product.on_hand = 5
    end
  end
  ...
  context '#create' do
    before do
      ...
      @product = Merriweather::Product.new(:name => "Foo", :price => 1.99)
    end
  ...
  context '#has_stock?' do
    let(:product) do
      product = stub_model(Merriweather::Product)
      product.stub :master => stub_model(Merriweather::Variant)
      product
    end
  ...
  context "#images" do
    ...
    before do
      ...
      Merriweather::Image.create({:viewable_id => product.master.id, :viewable_type => 'Merriweather::Variant',        :alt => "position 2", :attachment => image, :position => 2})
      Merriweather::Image.create({:viewable_id => product.master.id, :viewable_type => 'Merriweather::Variant',        :alt => "position 1", :attachment => image, :position => 1})
      Merriweather::Image.create({:viewable_id => product.master.id, :viewable_type => 'ThirdParty::Extension', :alt => "position 1", :attachment => image, :position => 2})
    end
