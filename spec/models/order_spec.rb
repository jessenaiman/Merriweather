...
class FakeCalculator < Merriweather::Calculator
...
describe Merriweather::Order do
  before(:each) do
    reset_Merriweather_preferences
    Merriweather::Gateway.create({:name => 'Test', :active => true, :environment => 'test', :description => 'foofah'}, :without_protection => true)
  end

  let(:user) { stub_model(Merriweather::LegacyUser, :email => "Merriweather@example.com") }
  let(:order) { stub_model(Merriweather::Order, :user => user) }
  let(:gateway) { Merriweather::Gateway::Bogus.new({:name => "Credit Card", :active => true}, :without_protection => true) }

  before do
    Merriweather::Gateway.stub :current => gateway
    Merriweather::LegacyUser.stub(:current => mock_model(Merriweather::LegacyUser, :id => 123))
  end

  context "#products" do
    before :each do
      @variant1 = mock_model(Merriweather::Variant, :product => "product1")
      @variant2 = mock_model(Merriweather::Variant, :product => "product2")
      @line_items = [mock_model(Merriweather::LineItem, :variant => @variant1, :variant_id => @variant1.id, :quantity => 1),
                     mock_model(Merriweather::LineItem, :variant => @variant2, :variant_id => @variant2.id, :quantity => 2)]
    ...
    it "gets the quantity of a given variant" do
      ...
      @variant3 = mock_model(Merriweather::Variant, :product => "product3")
      order.quantity_of(@variant3).should == 0
    end

    it "can find a line item matching a given variant" do
      order.find_line_item_by_variant(@variant1).should_not be_nil
      order.find_line_item_by_variant(mock_model(Merriweather::Variant)).should be_nil
    end
  end
  ...
  context "#create" do
    it "should assign an order number" do
      order = Merriweather::Order.create
      order.number.should_not be_nil
    end
  end

  context "#finalize!" do
    let(:order) { Merriweather::Order.create }
    it "should set completed_at" do
      order.should_receive(:touch).with(:completed_at)
      order.finalize!
    end
    it "should sell inventory units" do
      Merriweather::InventoryUnit.should_receive(:assign_opening_inventory).with(order)
      order.finalize!
    end

    after { Merriweather::Config.set :track_inventory_levels => true }
    it "should not sell inventory units if track_inventory_levels is false" do
      Merriweather::Config.set :track_inventory_levels => false
      Merriweather::InventoryUnit.should_not_receive(:sell_units)
      order.finalize!
    end

    it "should send an order confirmation email" do
      mail_message = mock "Mail::Message"
      Merriweather::OrderMailer.should_receive(:confirm_email).with(order).and_return mail_message
      mail_message.should_receive :deliver
      order.finalize!
    end

    it "should continue even if confirmation email delivery fails" do
      Merriweather::OrderMailer.should_receive(:confirm_email).with(order).and_raise 'send failed!'
      order.finalize!
    end

    it "should freeze all adjustments" do
      Merriweather::OrderMailer.stub_chain :confirm_email, :deliver
      adjustment1 = mock_model(Merriweather::Adjustment, :mandatory => true)
      adjustment2 = mock_model(Merriweather::Adjustment, :mandatory => false)
  ...
  context "#process_payments!" do
    it "should process the payments" do
      order.stub!(:payments).and_return([mock(Merriweather::Payment)])
  ...
  context "#backordered?" do
    it "should indicate whether any units in the order are backordered" do
      ...
      order.stub_chain(:inventory_units, :backordered).and_return [mock_model(Merriweather::InventoryUnit)]
  ...
  context "with adjustments" do
    let(:adjustment1) { mock_model(Merriweather::Adjustment, :amount => 5) }
    let(:adjustment2) { mock_model(Merriweather::Adjustment, :amount => 10) }
  ...
  context "rate_hash" do
    let(:shipping_method_1) { mock_model Merriweather::ShippingMethod, :name => 'Air Shipping', :id => 1, :calculator => mock('calculator') }
    let(:shipping_method_2) { mock_model Merriweather::ShippingMethod, :name => 'Ground Shipping', :id => 2, :calculator => mock('calculator') }
  ...
  context "insufficient_stock_lines" do
    let(:line_item) { mock_model Merriweather::LineItem, :insufficient_stock? => true }
  ...
  context "clear_adjustments" do
    before do
      @order = Merriweather::Order.new
    end

    it "should destroy all previous tax adjustments" do
      adjustment = mock_model(Merriweather::Adjustment)
  ...
  context "#tax_zone" do
    ...
    let(:order) { Merriweather::Order.create(:ship_address => ship_address, :bill_address => bill_address) }
    let(:zone) { Factory :zone }

    context "when no zones exist" do
      before { Merriweather::Zone.destroy_all }
    ...
    context "when :tax_using_ship_address => true" do
      before { Merriweather::Config.set(:tax_using_ship_address => true) }

      it "should calculate using ship_address" do
        Merriweather::Zone.should_receive(:match).at_least(:once).with(ship_address)
        Merriweather::Zone.should_not_receive(:match).with(bill_address)
        order.tax_zone
      end
    end

    context "when :tax_using_ship_address => false" do
      before { Merriweather::Config.set(:tax_using_ship_address => false) }

      it "should calculate using bill_address" do
        Merriweather::Zone.should_receive(:match).at_least(:once).with(bill_address)
        Merriweather::Zone.should_not_receive(:match).with(ship_address)
        order.tax_zone
      end
    end
    ...
    context "when there is a default tax zone" do
      before do
        ...
        Merriweather::Zone.stub :default_tax => @default_zone
      end

      context "when there is a matching zone" do
        before { Merriweather::Zone.stub(:match => zone) }
      ...
      context "when there is no matching zone" do
        before { Merriweather::Zone.stub(:match => nil) }
    ...
    context "when no default tax zone" do
      before { Merriweather::Zone.stub :default_tax => nil }
  ...
  context "#price_adjustment_totals" do
    before { @order = Merriweather::Order.create! }
    ...
    context "when there are two adjustments with different labels" do
      let(:adj1) { mock_model Merriweather::Adjustment, :amount => 10, :label => "Foo" }
      let(:adj2) { mock_model Merriweather::Adjustment, :amount => 20, :label => "Bar" }
    ...
    context "when there are two adjustments with one label and a single adjustment with another" do
      let(:adj1) { mock_model Merriweather::Adjustment, :amount => 10, :label => "Foo" }
      let(:adj2) { mock_model Merriweather::Adjustment, :amount => 20, :label => "Bar" }
      let(:adj3) { mock_model Merriweather::Adjustment, :amount => 40, :label => "Bar" }
  ...
  context "#exclude_tax?" do
    ...
    context "when prices include tax" do
      before { Merriweather::Config.set(:prices_inc_tax => true) }
    ...
    context "when prices do not include tax" do
      before { Merriweather::Config.set(:prices_inc_tax => false) }
  ...
  context "#add_variant" do
    it "should update order totals" do
      ...
      product = Merriweather::Product.create!(:name => 'Test', :sku => 'TEST-1', :price => 22.25)
  ...
  context "empty!" do
    it "should clear out all line items and adjustments" do
      order = stub_model(Merriweather::Order)
      order.stub(:line_items => line_items = [])
  ...
  context "#display_total" do
    ...
    context "with display_currency set to true" do
      before { Merriweather::Config[:display_currency] = true }
    ...
    context "with display_currency set to false" do
      before { Merriweather::Config[:display_currency] = false }
