...
describe Merriweather::ReturnAuthorization do
  let(:inventory_unit) { Merriweather::InventoryUnit.create({:variant => mock_model(Merriweather::Variant)}, :without_protection => true) }
  let(:order) { mock_model(Merriweather::Order, :inventory_units => [inventory_unit], :awaiting_return? => false) }
  let(:return_authorization) { Merriweather::ReturnAuthorization.new({:order => order}, :without_protection => true) }
  ...
  context "add_variant" do
    ...
    context "on rma that already has inventory_units" do
      let(:inventory_unit_2)  { Merriweather::InventoryUnit.create({:variant => inventory_unit.variant}, :without_protection => true) }
  ...
  context "receive!" do
    before  do
      ...
      Merriweather::Adjustment.stub(:create)
      order.stub(:update!)
    end
    ...
    it "should add credit for specified amount" do
      ...
      Merriweather::Adjustment.should_receive(:new).with(:amount => -20, :label => I18n.t(:rma_credit)).and_return(mock_adjustment)
      return_authorization.receive!
    end
