...
describe Merriweather::TaxRate do
  context "match" do
    let(:order) { create(:order) }
    let(:country) { create(:country) }
    let(:tax_category) { create(:tax_category) }
    let(:calculator) { Merriweather::Calculator::FlatRate.new }


    it "should return an empty array when tax_zone is nil" do
      order.stub :tax_zone => nil
      Merriweather::TaxRate.match(order).should == []
    end

    context "when no rate zones match the tax zone" do
      before do
        Merriweather::TaxRate.create({:amount => 1, :zone => create(:zone, :name => 'other_zone')}, :without_protection => true)
      end
      ...
      context "when there is no default tax zone" do
        ...
        it "should return the rate that matches the rate zone" do
          rate = Merriweather::TaxRate.create({ :amount => 1, :zone => @zone, :tax_category => tax_category,
                                       :calculator => calculator }, :without_protection => true)

          order.stub :tax_zone => @zone
          Merriweather::TaxRate.match(order).should == [rate]
        end

        it "should return all rates that match the rate zone" do
          rate1 = Merriweather::TaxRate.create({:amount => 1, :zone => @zone, :tax_category => tax_category,
                                        :calculator => calculator}, :without_protection => true)
          rate2 = Merriweather::TaxRate.create({:amount => 2, :zone => @zone, :tax_category => tax_category,
                                        :calculator => Merriweather::Calculator::FlatRate.new}, :without_protection => true)

          order.stub :tax_zone => @zone
          Merriweather::TaxRate.match(order).should == [rate1, rate2]
        end
  ...
  context "adjust" do
    ...
    let(:rate_1) { stub_model(Merriweather::TaxRate) }
    let(:rate_2) { stub_model(Merriweather::TaxRate) }

    before do
      Merriweather::TaxRate.stub :match => [rate_1, rate_2]
    end

    it "should apply adjustments for two tax rates to the order" do
      rate_1.should_receive(:adjust)
      rate_2.should_receive(:adjust)
      Merriweather::TaxRate.adjust(order)
    end
  end
  ...
  context "default" do
    ...
    context "when there is no default tax_category" do
      ...
      it "should return 0" do
        Merriweather::TaxRate.default.should == 0
      end
    end
    ...
    context "when there is a default tax_category" do
      ...
      context "when the default category has tax rates in the default tax zone" do
        before(:each) do
          Merriweather::Config[:default_country_id] = country.id
          @zone = create(:zone, :name => "Country Zone", :default_tax => true)
          @zone.zone_members.create(:zoneable => country)
          rate = Merriweather::TaxRate.create({:amount => 1, :zone => @zone, :tax_category => tax_category, :calculator => calculator}, :without_protection => true)
        end

        it "should return the correct tax_rate" do
          Merriweather::TaxRate.default.to_f.should == 1.0
        end
      end
  ...
  context "#adjust" do
    before do
      @category    = Merriweather::TaxCategory.create :name => "Taxable Foo"
      @category2   = Merriweather::TaxCategory.create(:name => "Non Taxable")
      @calculator  = Merriweather::Calculator::DefaultTax.new
      @rate        = Merriweather::TaxRate.create({:amount => 0.10, :calculator => @calculator, :tax_category => @category}, :without_protection => true)
    ...
    context "when order has one taxable line item" do
      ...
      context "when price includes tax" do
        ...
        context "when zone is contained by default tax zone" do
          before { Merriweather::Zone.stub_chain :default_tax, :contains? => true }
        ...
        context "when zone is not contained by default tax zone" do
          before { Merriweather::Zone.stub_chain :default_tax, :contains? => false }
