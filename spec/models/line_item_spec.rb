...
describe Merriweather::LineItem do
  before(:each) do
    reset_Merriweather_preferences
  end

  let(:variant) { mock_model(Merriweather::Variant, :count_on_hand => 95, :price => 9.99) }
  let(:line_item) { Merriweather::LineItem.new(:quantity => 5) }
  let(:order) do
    shipments = mock(:shipments, :reduce => 0)
    mock_model(Merriweather::Order, :line_items => [line_item],
                             :inventory_units => [],
                             :shipments => shipments,
                             :completed? => true,
                             :update! => true)
  ...
  before do
    ...
    Merriweather::Config.set :allow_backorders => true
  end

  context '#save' do
    it 'should update inventory and totals' do
      Merriweather::InventoryUnit.stub(:increase)
    ...
    context 'when order#completed? is true' do
      ...
      context 'and line_item is a new record' do
        ...
        it 'should increase inventory' do
          Merriweather::InventoryUnit.stub(:increase)
          Merriweather::InventoryUnit.should_receive(:increase).with(order, variant, 5)
          line_item.save
        end
      end
      ...
      context 'and quantity is increased' do
        ...
        it 'should increase inventory' do
          Merriweather::InventoryUnit.should_not_receive(:decrease)
          Merriweather::InventoryUnit.should_receive(:increase).with(order, variant, 1)
          line_item.save
        end
      end
      ...
      context 'and quantity is decreased' do
        ...
        it 'should decrease inventory' do
          ...
          Merriweather::InventoryUnit.should_receive(:decrease).with(order, variant, 2)
          line_item.save
        end
      end
  ...
  context '#destroy' do
    context 'when order.completed? is true' do
      it 'should remove inventory' do
        Merriweather::InventoryUnit.should_receive(:decrease).with(order, variant, 5)
        line_item.destroy
      end
    end
    ...
    context 'with inventory units' do
      let(:inventory_unit) { mock_model(Merriweather::InventoryUnit, :variant_id => variant.id, :shipped? => false) }
      before do
  ...
  context 'after shipment made' do
    before do
      shipping_method = mock_model(Merriweather::ShippingMethod, :calculator => mock(:calculator))
      shipment = Merriweather::Shipment.new :order => order, :shipping_method => shipping_method
      shipment.stub(:state => 'shipped')
      shipped_inventory_units = 5.times.map { Merriweather::InventoryUnit.new({ :variant => line_item.variant, :state => 'shipped' }, :without_protection => true) }
      unshipped_inventory_units = 2.times.map { Merriweather::InventoryUnit.new({ :variant => line_item.variant, :state => 'sold' }, :without_protection => true) }
