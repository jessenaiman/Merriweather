...
describe Merriweather::Order do
  ...
  context "#update!" do
    let(:line_items) { [mock_model(Merriweather::LineItem, :amount => 5) ]}
    ...
    context "when payments are insufficient" do
      let(:payments) { mock "payments", :completed => [], :first => mock_model(Merriweather::Payment, :checkout? => false) }
    ...
    context "when there are shipments" do
      let(:shipments) { [mock_model(Merriweather::Shipment, :update! => nil), mock_model(Merriweather::Shipment, :update! => nil)] }
      before do
      ...
      it "should set the correct shipment_state (when all shipments are shipped)" do
        shipments.stub :shipped => [mock_model(Merriweather::Shipment), mock_model(Merriweather::Shipment)]
        order.update!
        order.shipment_state.should == "shipped"
      end

      it "should set the correct shipment_state (when some units are backordered)" do
        shipments.stub :shipped => [mock_model(Merriweather::Shipment)]
      ...
      it "should set the shipment_state to partial (when some of the shipments have shipped)" do
        shipments.stub :shipped => [mock_model(Merriweather::Shipment)]
        shipments.stub :ready => [mock_model(Merriweather::Shipment)]
        order.update!
        order.shipment_state.should == "partial"
      end

      it "should set the correct shipment_state (when some of the shipments are ready)" do
        shipments.stub :ready => [mock_model(Merriweather::Shipment), mock_model(Merriweather::Shipment)]
        order.update!
        order.shipment_state.should == "ready"
      end

      it "should set the shipment_state to pending (when all shipments are pending)" do
        shipments.stub :pending => [mock_model(Merriweather::Shipment), mock_model(Merriweather::Shipment)]
        order.update!
    ...
    context "when there are update hooks" do
      before { Merriweather::Order.register_update_hook :foo }
      after { Merriweather::Order.update_hooks.clear }
    ...
    context "when there is a single checkout payment" do
      before { order.stub(:payment => mock_model(Merriweather::Payment, :checkout? => true, :amount => 11), :total => 22) }
    ...
    it "should call update! on every shipment when Order#update!" do
      shipment = stub_model(Merriweather::Shipment, :order => order)
  ...
  context "#update_totals" do
    it "should set item_total to the sum of line_item amounts" do
      line_items = [ mock_model(Merriweather::LineItem, :amount => 100), mock_model(Merriweather::LineItem, :amount => 50) ]
      order.stub(:line_items => line_items)
      order.update!
      order.item_total.should == 150
    end
    it "should set payments_total to the sum of completed payment amounts" do
      payments = [ mock_model(Merriweather::Payment, :amount => 100, :checkout? => false), mock_model(Merriweather::Payment, :amount => -10, :checkout? => false) ]
