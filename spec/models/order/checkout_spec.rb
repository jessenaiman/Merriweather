...
describe Merriweather::Order do
  ...
  context "with default state machine" do
    it "has the following transitions" do
      ...
      transitions.each do |transition|
        transition = Merriweather::Order.find_transition(:from => transition.keys.first, :to => transition.values.first)
        transition.should_not be_nil
      end
    end

    it "does not have a transition from delivery to confirm" do
      transition = Merriweather::Order.find_transition(:from => :delivery, :to => :confirm)
  ...
  context "subclassed order" do
    ...
    #   the other test pass
    class SubclassedOrder < Merriweather::Order
      checkout_flow do
        go_to_state :payment
  ...
  context "re-define checkout flow" do
    before do
      @old_checkout_flow = Merriweather::Order.checkout_flow
      Merriweather::Order.class_eval do
        checkout_flow do
          go_to_state :payment
          go_to_state :complete
        end
    ...
    after do
      Merriweather::Order.checkout_flow = @old_checkout_flow
    end

    it "should not keep old event transitions when checkout_flow is redefined" do
      Merriweather::Order.next_event_transitions.should == [{:cart=>:payment}, {:payment=>:complete}]
    end

    it "should not keep old events when checkout_flow is redefined" do
      state_machine = Merriweather::Order.state_machine
