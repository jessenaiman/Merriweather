...
describe Merriweather::Order do
  ...
  before do
    ...
    Merriweather::Order.define_state_machine!
  ...
  context "#next!" do
    context "when current state is confirm" do
      ...
      it "should finalize order when transitioning to complete state" do
       ...
       context "when credit card payment fails" do
         before do
           order.stub(:process_payments!).and_raise(Merriweather::Core::GatewayError)
         end

         context "when not configured to allow failed payments" do
            before do
              Merriweather::Config.set :allow_checkout_on_gateway_error => false
            end
         ...
         context "when configured to allow failed payments" do
           before do
             Merriweather::Config.set :allow_checkout_on_gateway_error => true
           end
    ...
    context "when current state is address" do
      ...
      it "adjusts tax rates when transitioning to delivery" do
        ...
        Merriweather::TaxRate.should_receive(:adjust).twice
        order.next!
      end
    end
  ...
  context "#cancel" do
    let!(:variant) { stub_model(Merriweather::Variant, :on_hand => 0) }
    let!(:inventory_units) { [stub_model(Merriweather::InventoryUnit, :variant => variant),
                              stub_model(Merriweather::InventoryUnit, :variant => variant) ]}
    let!(:shipment) do
      shipment = stub_model(Merriweather::Shipment)
    ...
    before do
      order.stub :line_items => [stub_model(Merriweather::LineItem, :variant => variant, :quantity => 2)]
    ...
    it "should send a cancel email" do
      ...
      Merriweather::OrderMailer.should_receive(:cancel_email).with(order).and_return mail_message
    ...
    context "restocking inventory" do
      before do
        ...
        Merriweather::OrderMailer.stub(:cancel_email).and_return(mail_message = stub)
        mail_message.stub :deliver
      end

      # Regression fix for #729
      specify do
        Merriweather::InventoryUnit.should_receive(:decrease).with(order, variant, 2).once
        order.cancel!
      end
    end
  ...
  context "#resume" do
    before do
      order.stub :email => "user@Merriweathercommerce.com"
      order.stub :state => "canceled"
      order.stub :allow_resume? => true
    end

    context "unstocks inventory" do
      let(:variant) { stub_model(Merriweather::Variant) }

      before do
        shipment = stub_model(Merriweather::Shipment)
        line_item = stub_model(Merriweather::LineItem, :variant => variant, :quantity => 2)
        order.stub :line_items => [line_item]
        order.line_items.stub :find_by_variant_id => line_item

        order.stub :shipments => [shipment]
        shipment.stub :inventory_units => [stub_model(Merriweather::InventoryUnit, :variant => variant),
                                           stub_model(Merriweather::InventoryUnit, :variant => variant) ]
      end

      specify do
        Merriweather::InventoryUnit.should_receive(:increase).with(order, variant, 2).once
        order.resume!
      end
    end
