...
describe Merriweather::Address do
  describe "clone" do
    it "creates a copy of the address with the exception of the id, updated_at and created_at attributes" do
      ...
      original = create(:address,
                         ...
                         :city => 'city',
                         :country => Merriweather::Country.first,
                         :firstname => 'firstname',
                         :lastname => 'lastname',
                         :company => 'company',
                         :phone => 'phone',
  ...
  context "validation" do
    ...
    let(:country) { mock_model(Merriweather::Country, :states => [state]) }
    let(:state) { stub_model(Merriweather::State, :name => 'maryland', :abbr => 'md') }
    ...
    it "state is entered but country does not contain that state" do
      address.state = state
      address.country = stub_model(Merriweather::Country)
      address.valid?
    ...
    it "address_requires_state preference is false" do
      Merriweather::Config.set :address_requires_state => false
      address.state = nil
      address.state_name = nil
      address.should be_valid
    end
  ...
  context ".default" do
    before do
      @default_country_id = Merriweather::Config[:default_country_id]
      new_country = create(:country)
      Merriweather::Config[:default_country_id] = new_country.id
    end

    after do
      Merriweather::Config[:default_country_id] = @default_country_id
    end
    it "sets up a new record with Merriweather::Config[:default_country_id]" do
      Merriweather::Address.default.country.should == Merriweather::Country.find(Merriweather::Config[:default_country_id])
    end

    # Regression test for #1142
    it "uses the first available country if :default_country_id is set to an invalid value" do
      Merriweather::Config[:default_country_id] = "0"
      Merriweather::Address.default.country.should == Merriweather::Country.first
    end
  end

  context '#full_name' do
    context 'both first and last names are present' do
      let(:address) { stub_model(Merriweather::Address, :firstname => 'Michael', :lastname => 'Jackson') }
      specify { address.full_name.should == 'Michael Jackson' }
    end

    context 'first name is blank' do
      let(:address) { stub_model(Merriweather::Address, :firstname => nil, :lastname => 'Jackson') }
      specify { address.full_name.should == 'Jackson' }
    end

    context 'last name is blank' do
      let(:address) { stub_model(Merriweather::Address, :firstname => 'Michael', :lastname => nil) }
      specify { address.full_name.should == 'Michael' }
    end

    context 'both first and last names are blank' do
      let(:address) { stub_model(Merriweather::Address, :firstname => nil, :lastname => nil) }
  ...
  context '#state_text' do
    context 'state is blank' do
      let(:address) { stub_model(Merriweather::Address, :state => nil, :state_name => 'virginia') }
      specify { address.state_text.should == 'virginia' }
    end

    context 'both name and abbr is present' do
      let(:state) { stub_model(Merriweather::State, :name => 'virginia', :abbr => 'va') }
      let(:address) { stub_model(Merriweather::Address, :state => state) }
      specify { address.state_text.should == 'va' }
    end

    context 'only name is present' do
      let(:state) { stub_model(Merriweather::State, :name => 'virginia', :abbr => nil) }
      let(:address) { stub_model(Merriweather::Address, :state => state) }
      specify { address.state_text.should == 'virginia' }
    end
