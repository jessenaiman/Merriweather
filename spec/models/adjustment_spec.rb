...
describe Merriweather::Adjustment do

  let(:order) { mock_model(Merriweather::Order, :update! => nil) }
  let(:adjustment) { Merriweather::Adjustment.new }
  ...
  context "#save" do
    it "should call order#update!" do
      adjustment = Merriweather::Adjustment.new({:adjustable => order, :amount => 10, :label => "Foo"}, :without_protection => true)
  ...
  context "#eligible_for_originator?" do
    ...
    context "with originator that doesn't have 'eligible?'" do
      before { adjustment.originator = mock_model(Merriweather::TaxRate) }
      specify { adjustment.should be_eligible_for_originator }
    end
    context "with originator that has 'eligible?'" do
      let(:originator) { Merriweather::TaxRate.new }
