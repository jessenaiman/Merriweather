...
describe Merriweather::CreditCard do
  ...
  let(:credit_card) { Merriweather::CreditCard.new }
  ...
  before(:each) do
    ...
    @payment_gateway = mock_model(Merriweather::PaymentMethod,
  ...
  context "#can_capture?" do
    it "should be true if payment state is pending" do
      payment = mock_model(Merriweather::Payment, :state => 'pending', :created_at => Time.now)
      credit_card.can_capture?(payment).should be_true
    end
  end

  context "when transaction is more than 12 hours old" do
    let(:payment) { mock_model(Merriweather::Payment, :state => "completed",
                                               :created_at => Time.now - 14.hours,
                                               :amount => 99.00,
                                               :credit_allowed => 100.00,
                                               :order => mock_model(Merriweather::Order, :payment_state => 'credit_owed')) }
  ...
  context "#save" do
    ...
    let!(:persisted_card) { Merriweather::CreditCard.find(credit_card.id) }
  ...
  context "#Merriweather_cc_type" do
    before do
      credit_card.attributes = valid_credit_card_attributes
    end
    ...
    context "in development mode" do
      ...
      it "should return visa" do
        credit_card.save
        credit_card.Merriweather_cc_type.should == "visa"
      end
    end
    ...
    context "in production mode" do
      ...
      it "should return the actual cc_type for a valid number" do
        ...
        credit_card.save
        credit_card.Merriweather_cc_type.should == "american_express"
      end
    end
  end
  ...
  context "#set_card_type" do
    ...
    it "stores the credit card type after validation" do
      ...
      credit_card.save
      credit_card.Merriweather_cc_type.should == "discover"
    end
    ...
    it "does not overwrite the credit card type when loaded and saved" do
      ...
      credit_card.save
      credit_card.Merriweather_cc_type.should == "master"
    end
  end
