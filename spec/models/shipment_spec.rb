...
describe Merriweather::Shipment do
  before(:each) do
    reset_Merriweather_preferences
  end

  let(:order) { mock_model Merriweather::Order, :backordered? => false }
  let(:shipping_method) { mock_model Merriweather::ShippingMethod, :calculator => mock('calculator') }
  let(:shipment) do
  ...
  let(:charge) { mock_model Merriweather::Adjustment, :amount => 10, :source => shipment }
  ...
  context "#update!" do
    ...
    shared_examples_for "pending if backordered" do
      it "should have a state of pending if backordered" do
        shipment.stub(:inventory_units => [mock_model(Merriweather::InventoryUnit, :backordered? => true)] )
  ...
  context "when track_inventory is false" do
    ...
    it "should not use the line items from order when track_inventory_levels is false" do
      line_items = [mock_model(Merriweather::LineItem)]
  ...
  context "when order is completed" do
    ...
    context "with inventory tracking" do
      before { Merriweather::Config.set :track_inventory_levels => true }
  ...
  context "#ship" do
    ...
    it "should send a shipment email" do
      ...
      Merriweather::ShipmentMailer.should_receive(:shipped_email).with(shipment).and_return mail_message
