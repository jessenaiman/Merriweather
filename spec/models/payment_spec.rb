...
describe Merriweather::Payment do
  let(:order) do
    order = Merriweather::Order.new(:bill_address => Merriweather::Address.new,
                             :ship_address => Merriweather::Address.new)
  end

  let(:gateway) do
    gateway = Merriweather::Gateway::Bogus.new({:environment => 'test', :active => true}, :without_protection => true)
    gateway.stub :source_required => true
    gateway
  end

  let(:card) do
    mock_model(Merriweather::CreditCard, :number => "4111111111111111",
                                  :has_payment_profile? => true)
  end

  let(:payment) do
    payment = Merriweather::Payment.new
    payment.source = card
    payment.order = order
  ...
  context "processing" do
    ...
    context "#process!" do
      it "should purchase if with auto_capture" do
        Merriweather::Config[:auto_capture] = true
        payment.should_receive(:purchase!)
        payment.process!
      end

      it "should authorize without auto_capture" do
        Merriweather::Config[:auto_capture] = false
    ...
    context "#authorize" do
      ...
      context "when gateway does not match the environment" do
        it "should raise an exception" do
          ...
          lambda { payment.authorize! }.should raise_error(Merriweather::Core::GatewayError)
        end
      end
      ...
      context "if unsucessful" do
        it "should mark payment as failed" do
          ...
          lambda {
            payment.authorize!
          }.should raise_error(Merriweather::Core::GatewayError)
        end
      end
    end
    ...
    context "purchase" do
      ...
      context "when gateway does not match the environment" do
        it "should raise an exception" do
          gateway.stub :environment => "foo"
          lambda { payment.purchase!  }.should raise_error(Merriweather::Core::GatewayError)
        end
      end
      ...
      context "when payment is pending" do
        ...
        context "if unsucessful" do
          it "should not make payment complete" do
            ...
            lambda { payment.capture! }.should raise_error(Merriweather::Core::GatewayError)
          end
        end
      end
    end
    ...
    context "#void" do
      ...
      context "when gateway does not match the environment" do
        it "should raise an exception" do
          gateway.stub :environment => "foo"
          lambda { payment.void_transaction! }.should raise_error(Merriweather::Core::GatewayError)
        end
      end
    ...
    context "#credit" do
      ...
      context "when gateway does not match the environment" do
        it "should raise an exception" do
          gateway.stub :environment => "foo"
          lambda { payment.credit! }.should raise_error(Merriweather::Core::GatewayError)
        end
      end

      context "when response is sucesssful" do
        it "should create an offsetting payment" do
          Merriweather::Payment.should_receive(:create)
          payment.credit!
        end
  ...
  context "when response is unsucessful" do
    it "should not create a payment" do
      ...
      Merriweather::Payment.should_not_receive(:create)
      lambda { payment.credit! }.should raise_error(Merriweather::Core::GatewayError)
    end
  end
  ...
  context "with source required" do
    context "raises an error if no source is specified" do
      ...
      specify do
        lambda { payment.process! }.should raise_error(Merriweather::Core::GatewayError, I18n.t(:payment_processing_failed))
      end
    end
  end
  ...
  context "with source optional" do
    context "raises no error if source is not specified" do
      ...
      specify do
        lambda { payment.process! }.should_not raise_error(Merriweather::Core::GatewayError)
      end
    end
  end
  ...
  context "#save" do
    it "should call order#update!" do
      payment = Merriweather::Payment.create({:amount => 100, :order => order}, :without_protection => true)
    ...
    context "when profiles are supported" do
      ...
      context "when there is an error connecting to the gateway" do
        it "should call gateway_error " do
          ...
          lambda { Merriweather::Payment.create({:amount => 100, :order => order, :source => card, :payment_method => gateway}, :without_protection => true) }.should raise_error(Merriweather::Core::GatewayError)
        end
      end
      ...
      context "when successfully connecting to the gateway" do
        it "should create a payment profile" do
          ...
          payment = Merriweather::Payment.create({:amount => 100, :order => order, :source => card, :payment_method => gateway}, :without_protection => true)
        end
      end
  ...
  context "#build_source" do
    it "should build the payment's source" do
      ...
      payment = Merriweather::Payment.new(params, :without_protection => true)
