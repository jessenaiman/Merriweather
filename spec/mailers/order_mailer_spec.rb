...
describe Merriweather::OrderMailer do
  include EmailSpec::Helpers
  include EmailSpec::Matchers

  let(:order) do
    order = stub_model(Merriweather::Order)
    product = stub_model(Merriweather::Product, :name => %Q{The "BEST" product})
    variant = stub_model(Merriweather::Variant, :product => product)
    line_item = stub_model(Merriweather::LineItem, :variant => variant, :order => order, :quantity => 1, :price => 5)
    order.stub(:line_items => [line_item])
    order
  end

  it "doesn't aggressively escape double quotes in confirmation body" do
    confirmation_email = Merriweather::OrderMailer.confirm_email(order)
  ...
  context "only shows eligible adjustments in emails" do
    ...
    let!(:confirmation_email) { Merriweather::OrderMailer.confirm_email(order) }
    let!(:cancel_email) { Merriweather::OrderMailer.cancel_email(order) }
  ...
  context "emails must be translatable" do
    context "en locale" do
      ...
      context "cancel_email" do
        specify do
          cancel_email = Merriweather::OrderMailer.cancel_email(order)
