...
describe Merriweather::CheckoutController do
  let(:order) do 
    mock_model(Merriweather::Order, :checkout_allowed? => true,
                             :user => nil,
                             :email => nil,
                             :completed? => false,
  ...
  context "#edit" do
    ...
    it "should redirect to the cart path unless checkout_allowed?" do
      ...
      Merriweather_get :edit, { :state => "delivery" }
    ...
    it "should change to the requested state" do
      ...
      Merriweather_get :edit, { :state => "payment" }
    end

    it "should redirect to cart if order is completed" do
      order.stub(:completed? => true)
      Merriweather_get :edit, {:state => "address"}
      response.should redirect_to(Merriweather.cart_path)
    end

    it "should associate the order with a user" do
      user = double("User", :last_incomplete_Merriweather_order => nil)
      controller.stub(:Merriweather_current_user => user)
      order.should_receive(:associate_user!).with(user)
      Merriweather_get :edit, {}, :order_id => 1
    end

    it "should fire the Merriweather.user.signup event if user has just signed up" do
      user = double("User", :last_incomplete_Merriweather_order => nil)
      controller.stub(:Merriweather_current_user => user)
      controller.should_receive(:fire_event).with("Merriweather.user.signup", :user => user, :order => order)
      Merriweather_get :edit, {}, :Merriweather_user_signup => true
    end
  ...
  context "#update" do
    ...
    context "save successful" do
      ...
      it "should assign order" do
        ...
        Merriweather_post :update, {:state => "confirm"}
      ...
      context "with next state" do
        ...
        it "should advance the state" do
          ...
          Merriweather_post :update, {:state => "delivery"}
        end

        it "should redirect the next state" do
          order.stub :state => "payment"
          Merriweather_post :update, {:state => "delivery"}
          response.should redirect_to Merriweather.checkout_state_path("payment")
        end
        ...
        context "when in the confirm state" do
          ...
          it "should redirect to the order view" do
            Merriweather_post :update, {:state => "confirm"}
            response.should redirect_to Merriweather.order_path(order)
          end
          ...
          it "should remove completed order from the session" do
            Merriweather_post :update, {:state => "confirm"}, {:order_id => "foofah"}
    ...
    context "Merriweather::Core::GatewayError" do

      before do
        order.stub(:update_attributes).and_raise(Merriweather::Core::GatewayError)
        Merriweather_post :update, {:state => "whatever"}
      end
      ...
      it "should set appropriate flash message" do
        flash[:error].should == I18n.t(:Merriweather_gateway_error_flash_for_checkout)
      end
  ...
  context "When last inventory item has been purchased" do
    let(:product) { mock_model(Merriweather::Product, :name => "Amazing Object") }
    let(:variant) { mock_model(Merriweather::Variant, :on_hand => 0) }
    ...
    context "and back orders == false" do
      before do
        Merriweather_post :update, {:state => "payment"}
      end
      ...
      it "should set flash message for no inventory" do
        flash[:error].should == I18n.t(:Merriweather_inventory_error_flash_for_insufficient_quantity , :names => "'#{product.name}'" )
      end
