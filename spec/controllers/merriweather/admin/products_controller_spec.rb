...
describe Merriweather::Admin::ProductsController do
  stub_authorization!
  ...
  context "#index" do
    it "should not allow JSON request without a valid token" do
      ...
      expect {
        Merriweather_get :index, {:format => :json}
    ...
    it "should allow JSON request with invalid token if forgery protection is disabled" do
      controller.should_receive(:protect_against_forgery?).at_least(:once).and_return(false)
      Merriweather_get :index, {:authenticity_token => "XYZZY", :format => :json}
      response.should be_success
    end
    ...
    it "should allow JSON request with a valid token" do
      ...
      Merriweather_get :index, {:authenticity_token => "123456", :format => :json}
      response.should be_success
    end
    ...
    it "should allow JSON request when token has URL(+,&,=) characters" do
      ...
      Merriweather_get :index, {:authenticity_token => "1+2%3D3%264%275/6%3F", :format => :json}
      response.should be_success
    end
    ...
    it "can find a product by SKU" do
      ...
      Merriweather_get :index, :q => { :sku_start => "ABC123" }
    ...
    it "JSON request can find a product" do
      ...
      Merriweather_xhr_get :index, { :q => "Alpha", :format => :json }
      assigns[:collection].should_not be_empty
      assigns[:collection].should include(product)
    end
  end
  ...
  context "creating a product" do
    ...
    it "should create product" do
      Merriweather_get :new
      response.should render_template("admin/products/new")
    end

    it "should create product from prototype" do
      Merriweather_post :create, :product => product_attributes.merge(:prototype_id => prototype.id)
      product = Merriweather::Product.last
      response.should redirect_to(Merriweather.edit_admin_product_path(product))
    ...
    it "should create product from prototype with option values hash" do
      Merriweather_post :create, :product => product_attributes.merge(:prototype_id => prototype.id, :option_values_hash => option_values_hash)
      product = Merriweather::Product.last
      response.should redirect_to(Merriweather.edit_admin_product_path(product))
      option_values_hash.each do |option_type_id, option_value_ids|
        Merriweather::ProductOptionType.where(:product_id => product.id, :option_type_id => option_type_id).first.should_not be_nil
      end
  ...
  context "adding properties to a product" do
    ...
    specify do
      Merriweather_put :update, :id => product.to_param, :product => { :product_properties_attributes => { "1" => { :property_name => "Foo", :value => "bar" } } }
  ...
  context "destroying a product" do
    ...
    it "deletes all the variants (including master) for the product" do
      Merriweather_delete :destroy, :id => product
