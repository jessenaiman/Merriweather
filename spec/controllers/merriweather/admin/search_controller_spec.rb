...
describe Merriweather::Admin::SearchController do
  stub_authorization!
  ...
  it "can find a user by their email "do
    Merriweather_xhr_get :users, :q => user.email
    assigns[:users].should include(user)
  end

  it "can find a user by their ship address's first name" do
    Merriweather_xhr_get :users, :q => user.ship_address.firstname
    assigns[:users].should include(user)
  end

  it "can find a user by their ship address's last name" do
    Merriweather_xhr_get :users, :q => user.ship_address.lastname
    assigns[:users].should include(user)
  end

  it "can find a user by their bill address's first name" do
    Merriweather_xhr_get :users, :q => user.bill_address.firstname
    assigns[:users].should include(user)
  end

  it "can find a user by their bill address's last name" do
    Merriweather_xhr_get :users, :q => user.bill_address.lastname
    assigns[:users].should include(user)
  end
