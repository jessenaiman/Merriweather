module Merriweather
  ...
  module ProductFilters
    ...
    #
    Merriweather::Product.add_search_scope :price_range_any do |*opts|
      conds = opts.map {|o| Merriweather::ProductFilters.price_filter[:conds][o]}.reject {|c| c.nil?}
      scope = conds.shift
      conds.each do |new_scope|
        scope = scope.or(new_scope)
      end
      Merriweather::Product.joins(:master).where(scope)
    end

    def ProductFilters.format_price(amount)
      Merriweather::Money.new(amount)
    end

    def ProductFilters.price_filter
      v = Merriweather::Variant.arel_table
    ...
    if Merriweather::Property.table_exists?
      Merriweather::Product.add_search_scope :brand_any do |*opts|
        conds = opts.map {|o| ProductFilters.brand_filter[:conds][o]}.reject {|c| c.nil?}
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Merriweather::Product.with_property("brand").where(scope)
      end

      def ProductFilters.brand_filter
        brand_property = Merriweather::Property.find_by_name("brand")
        brands = Merriweather::ProductProperty.where(:property_id => brand_property).pluck(:value).uniq
        pp = Merriweather::ProductProperty.arel_table
    ...
    if Merriweather::Property.table_exists?
      Merriweather::Product.add_search_scope :selective_brand_any do |*opts|
        Merriweather::Product.brand_any(*opts)
      end

      def ProductFilters.selective_brand_filter(taxon = nil)
        taxon ||= Merriweather::Taxonomy.first.root
        brand_property = Merriweather::Property.find_by_name("brand")
        scope = Merriweather::ProductProperty.where(:property_id => brand_property).
                                       joins(:product => :taxons).
                                       where("#{Merriweather::Taxon.table_name}.id" => [taxon] + taxon.descendants).
                                       scoped
    ...
    def ProductFilters.taxons_below(taxon)
      return Merriweather::ProductFilters.all_taxons if taxon.nil?
    ...
    def ProductFilters.all_taxons
      taxons = Merriweather::Taxonomy.all.map {|t| [t.root] + t.root.descendants }.flatten
      { :name   => "All taxons",
