...
require 'merriweather/core/custom_fixtures'

namespace :db do
  desc %q{Loads a specified fixture file:
For .yml/.csv use rake db:load_file[merriweather/filename.yml,/absolute/path/to/parent/]
For .rb       use rake db:load_file[/absolute/path/to/sample/filename.rb]}
  ...
  task :load_file , [:file, :dir] => :environment do |t, args|
    ...
    if %w{.csv .yml}.include? file.extname
      ...
      Merriweather::Core::Fixtures.create_fixtures(args.dir, file.to_s.sub(file.extname, ""))
    elsif file.exist?
  ...
  task :bootstrap  do
    ...
    load_defaults  = Merriweather::Country.count == 0
    ...
    if Rails.env.production? and Merriweather::Product.count > 0
    ...
    if load_sample
      ...
      Rake::Task["merriweather_sample:load"].invoke
    end
