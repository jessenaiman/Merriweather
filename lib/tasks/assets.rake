    ...
    task :relocate_images => :environment do
      ...
      %w{products taxons}.each do |directory|
        ...
        if depracated_assets_path.exist?
          new_assets_path = Rails.root.join("public/merriweather/", directory)
          ...
          Merriweather::Core::FileUtilz.mirror_files(depracated_assets_path.to_s, new_assets_path.to_s)
